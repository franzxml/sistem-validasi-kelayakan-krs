export default function Features() {
  return (
    <section id="fitur" className="py-24 md:py-48 bg-slate-950 text-white relative overflow-hidden scroll-mt-20">
      <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-10" />
      <div className="absolute top-0 left-0 w-full h-64 bg-gradient-to-b from-white to-transparent" />
      
      <div className="mx-auto max-w-7xl px-6 md:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-end mb-20 md:mb-32">
          <h2 className="text-5xl font-black tracking-[-0.04em] leading-[0.9] sm:text-7xl md:text-8xl">
            SISTEM <br /> <span className="text-blue-500 italic">YANG</span> <br /> MODERN.
          </h2>
          <p className="text-lg md:text-xl text-slate-400 font-medium max-w-md pb-4 border-l-4 border-blue-600 pl-6 md:pl-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10">
          {[
            ...
            <div 
              key={i} 
              className={`p-8 md:p-12 rounded-[2.5rem] md:rounded-[4rem] border-2 border-white/5 bg-white/5 flex flex-col h-[320px] md:h-[400px] justify-between ${i === 2 ? "lg:col-span-1" : ""} ${i === 4 ? "lg:col-span-2" : ""}`}
            >
              <div className="text-4xl md:text-6xl text-blue-500">{f.icon}</div>
              <div>
                <h3 className="text-2xl md:text-3xl font-black mb-2 md:mb-4 tracking-tighter">{f.title}</h3>
